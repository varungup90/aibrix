...
func TestClampReplicaInflightToRPS(t *testing.T) {
	assert.Equal(t, int64(0), clampReplicaInflightToRPS(0, 5))
	assert.Equal(t, int64(3), clampReplicaInflightToRPS(3, 0))
	assert.Equal(t, int64(5), clampReplicaInflightToRPS(5, 5))
	assert.Equal(t, int64(8), clampReplicaInflightToRPS(8, 5))
	assert.Equal(t, int64(5), clampReplicaInflightToRPS(3, 5))
	assert.Equal(t, int64(1), clampReplicaInflightToRPS(1, 0.5))
	assert.Equal(t, int64(6), clampReplicaInflightToRPS(5, 5.2))
}
...
func TestApplyConfigProfile_ReplicaInflight(t *testing.T) {
	...
	t.Run("inflight below replica rps is clamped", func(t *testing.T) {
		routingCtx := &types.RoutingContext{}
		applyConfigProfile(routingCtx, []*v1.Pod{podWithReplicaLimits("a", 5, 3)})
		assert.Equal(t, int64(5), routingCtx.ConfigProfile.RequestsInflight)
	...
	t.Run("fractional replica rps does not clamp inflight of 1", func(t *testing.T) {
		routingCtx := &types.RoutingContext{}
...
func TestEnforceReplicaInflight_AdmitThenReject(t *testing.T) {
	...
	mockCache.On("GetMetricValueByPod", "a", "ns", metrics.RealtimeNumRequestsRunning).
		Return(&metrics.SimpleMetricValue{Value: 1}, nil).Once()
...
func TestEnforceReplicaInflight_TwoPodsIndependent(t *testing.T) {
	...
	mockCache.On("GetMetricValueByPod", "b", "ns", metrics.RealtimeNumRequestsRunning).
		Return(&metrics.SimpleMetricValue{Value: 0}, nil).Once()
...
// the target pod (just selected from the same cache) doesn't yet have a running-request
// metric recorded -- there is no external dependency to fail closed on, so this admits
// the request.
func TestEnforceReplicaInflight_MetricLookupErrorFailsOpen(t *testing.T) {
	mockCache := &MockCache{}
	mockCache.On("GetMetricValueByPod", "a", "ns", metrics.RealtimeNumRequestsRunning).
		Return(&metrics.SimpleMetricValue{Value: 0}, assert.AnError)
	s := &Server{cache: mockCache}
	pod := podWithReplicaInflight("a", 1)
	routingCtx := types.NewRoutingContext(context.Background(), "", "m", "", "r1", "")
	applyConfigProfile(routingCtx, []*v1.Pod{pod})
...
func TestFilterSaturatedReplicaInflight(t *testing.T) {
	mockCache := &MockCache{}
	mockCache.On("GetMetricValueByPod", "a", "ns", metrics.RealtimeNumRequestsRunning).
		Return(&metrics.SimpleMetricValue{Value: 1}, nil)
	mockCache.On("GetMetricValueByPod", "b", "ns", metrics.RealtimeNumRequestsRunning).
...
func TestHandleRequestBody_ReplicaInflightAdmitThenReject(t *testing.T) {
	...
	// First request: filterSaturatedReplicaInflight's pre-routing check, enforceReplicaInflight's
	// post-routing check, and the request_start log's running-request field all read the pod's
	// running count while it's still below the limit (3 calls, all returning 0). Second request:
	// filterSaturatedReplicaInflight now sees the pod at the limit and rejects it before
	// enforceReplicaInflight is ever reached (1 call, returning 1).
	mockCache.On("GetMetricValueByPod", "a", "ns", metrics.RealtimeNumRequestsRunning).
		Return(&metrics.SimpleMetricValue{Value: 0}, nil).Times(3)
	mockCache.On("GetMetricValueByPod", "a", "ns", metrics.RealtimeNumRequestsRunning).
		Return(&metrics.SimpleMetricValue{Value: 1}, nil)
...
func TestHandleRequestBody_ReplicaInflightNotConsumedOnRoutingFailure(t *testing.T) {
	...
	// to pick a target below; neither call count nor per-pod value matters for this test.
...
func TestHandleRequestBody_ReplicaInflightCoexistsWithReplicaRPS(t *testing.T) {
	...
	// must be rejected by replica RPS (cap 1), not by inflight.
	mockCache.On("GetMetricValueByPod", "a", "ns", metrics.RealtimeNumRequestsRunning).
		Return(&metrics.SimpleMetricValue{Value: 0}, nil)
